...
The desktop shell owns page zoom. `Ctrl`/`Command` with `+` or `=` zooms in, `-` zooms out, and `0` restores 100%; the supported range stays between about 58% and 299%. Each newly loaded desktop window starts at 100% instead of inheriting Chromium's same-origin zoom state.
