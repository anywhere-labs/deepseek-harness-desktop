...
所有模式都将运行命令时所在的目录作为默认 workspace 根目录，以 65,536 字节渲染预算加载适用的 `AGENTS.md` 或 `CLAUDE.md` 指令，并使用内存 SQLite 会话内容索引。普通 profile 启动会监视 profile 与 home 两个 `cordis.patch.yml` 配置层的有效变更，并以事务方式重新应用；安全模式跳过两个 watcher。一次性运行模式通过有界关闭流程退出，该流程会 dispose 所有已启用的 watcher。
