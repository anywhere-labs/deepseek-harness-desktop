...
AI Buddy 是这套方式的第一个实践者：桌面壳本身就是一个普通 DSH 插件，与官方、第三方插件走同一条组合路径，没有任何特权。我们不是魔改上游源码做一个固定外壳，而是让"桌面"也成为插件生态里平等的一员。
...
内置插件市场已经让符合本倡议的插件更容易被发现、安装和信任。我们希望让“按规范开发”成为对每个作者都有利的选择，而不是额外的负担。
...
[DSH Community Market](../dsh-community-market/README.zh.md) 已内置于 AI Buddy，可在 **设置 > 插件** 中打开。目录收录只代表符合目录规则，不等于安全审核或推荐。
