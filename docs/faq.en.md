...
Current release installers support Windows x64, Apple Silicon macOS, and Linux amd64 (deb / rpm / AppImage). Intel Macs are not supported. Cross-platform compatibility code in the source tree does not imply that an installer has been released for that platform.
