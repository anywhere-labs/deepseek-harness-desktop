...
For a closer workbench setup, open **Settings > Plugins > Worker pack**: the default agent preset is Code. Recommended workspace plugins can be installed in one click or one at a time; nothing installs at launch. Office IM starts from official DingTalk Stream (`dsh-dingtalk-channel`) and the official WeCom AI Bot (`dsh-wecom`); that is a recommendation, not an allowlist, and other community channels still install. Narrow screens can later add `dsh-web-mobile`. No secrets are shipped. Configure external MCP servers on the **MCP** tab with the official client; no tokens are shipped. The Command Palette, local-model discovery, data-home migration, and remote entrance are optional. Remote access stays off by default and does not stream the Electron window.

## Are there official DingTalk and WeCom channels?

AI Buddy does not ship built-in channels and does not block community plugins. The worker pack uses official DingTalk Stream (`dsh-dingtalk-channel`, internal-app Client ID + Secret, no public callback) and the official WeCom AI Bot (`dsh-wecom`, Bot ID + Secret) as a starting point. Feishu, aggregators, and other community channels still install from the Plugin market or `dsh plugin add`. After installing, put credentials in the plugin config or the local credential store, then restart AI Buddy.
