...
这里是 AI Buddy 的产品与开发文档入口。根目录的 [`README.md`](../README.md) 适合第一次了解项目；本目录解释项目为什么存在、如何使用，以及如何为 AI Buddy 编写插件。想参与贡献？见[参与贡献](../CONTRIBUTING.md)。
...
| [为什么做 AI Buddy](why-desktop.md) | AI Buddy 与官方 Harness 的边界，以及为什么坚持插件化 |
...
| [Community Market](../dsh-community-market/README.zh.md) | 已内置的插件市场：发现、安装、卸载、目录来源和安全边界 |
...
`deepseek-harness/` 是固定版本的官方上游子模块。它自己的 README 和 `docs/` 属于上游项目，不能当作 AI Buddy 文档，也不在本仓库的产品文档统计中。

## 状态约定

文档会明确区分已实现能力、平台限制和 roadmap。兼容模式保留上游默认 Web 客户端；高级模式才安装 Desktop 自有的布局和原生材质。[`dsh-community-market`](../dsh-community-market/README.zh.md) 已内置：可在 **设置 > 插件** 的**插件市场**标签或侧边栏入口中发现、安装和管理插件。目录收录不等于安全审核。手机远程和 Channels 仍是独立 roadmap，不代表当前安装包已经提供这些产品入口。
