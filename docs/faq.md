...
想要更接近 Codex 的工作台时，打开 **设置 > 插件 > 工作套件**：默认 Agent 预设是 Code，推荐的工作区插件需从市场安装。办公 IM 只推荐钉钉官方 Stream（`dsh-dingtalk-channel`）和企业微信官方智能机器人（`dsh-wecom`），不会预装，也不会预置密钥。外部 MCP 服务器在 **MCP** 标签里用官方客户端配置，不会预置令牌。

## 有官方钉钉和企业微信吗？

没有内置通道。工作套件只推荐走官方协议的两个社区插件：钉钉用官方 Stream（`dsh-dingtalk-channel`，企业内部应用的 Client ID + Secret，无需公网回调），企业微信用官方智能机器人长连接（`dsh-wecom`，Bot ID + Secret）。Webhook 群通知、飞书和多家 IM 聚合插件不在推荐名单里。装完后把凭证写进插件配置或本机凭据服务，然后重启 AI Buddy。
