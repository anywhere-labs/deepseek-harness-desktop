...
想要更接近 Codex 的工作台时，打开 **设置 > 插件 > 工作套件**：默认 Agent 预设是 Code。推荐的工作区插件可以一键安装，或单独安装；不会开机自动装。办公 IM 的起点是钉钉官方 Stream（`dsh-dingtalk-channel`）和企业微信官方智能机器人（`dsh-wecom`）；这是推荐，不是白名单，其他社区通道仍可安装。窄屏可以把 `dsh-web-mobile` 当作后续可选推荐。不会预置密钥。外部 MCP 服务器在 **MCP** 标签里用官方客户端配置，不会预置令牌。命令面板、本机模型、数据目录迁移和远程入口都是可选能力；远程入口默认关闭，不会做像素流。
