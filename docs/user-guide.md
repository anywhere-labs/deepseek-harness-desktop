...
**设置 > 插件 > 工作套件** 列出推荐的工作区插件（例如 `dsh-better-sidebar` 和 `dsh-context`）。它们不会随应用预装，目录收录也不等于安全审核。插件市场默认不预选来源；你可以在工作套件页一键添加并选用 DSH 1024Store，再在**插件市场**里搜索这些 npm 包名。安装或启用插件后需要重启 AI Buddy。
