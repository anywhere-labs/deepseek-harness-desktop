...
一条包含润色指令与原文逐字内容的用户消息，以插件来源的 `user/message` 投递；模型回复以普通 `assistant/message` 落盘。指令固定原意、要求用原文语言写出更清晰完整的版本，并要求答案只能是润色后的文本——不带任何解释、前缀、引号或工具调用。返回文本绝不会被自动发送；调用方将其放入输入区供用户确认。
