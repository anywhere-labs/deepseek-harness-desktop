...
The cut is durable: persistence truncates the stored log (JSONL rewrites the surviving prefix atomically; SQLite deletes the tail rows in one transaction), and the live agent is disposed and resumed as the same identity, so the session continues from the cut point. A **cold** persisted session (no live agent) is truncated through persistence directly and stays cold — the next prompt resumes it with the full composition. The rewind ordering is deliberate — flush, dispose (which appends the inbox-splice tail), truncate, resume — so no delayed write can land past the cut.
...
Failures form a closed vocabulary: `session-not-found` (neither live nor persisted), `message-seq-out-of-range` (the seq is not a message in the current log), `no-turn` (the seq is inside no turn), and `rewind-failed` (the rewind threw; the session is untouched but files may be partially reverted).
