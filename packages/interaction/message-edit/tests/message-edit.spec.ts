...
describe('dsh-message-edit through the agent loop', () => {
  ...
  it('rewrites a settled user message in place on the model surface', async () => {
    ...
    const result = ctx.messageEdit.edit({
      sessionId: agent.id,
  ...
  it('edits an earlier message when later messages and assistant replies sit above it on the surface', async () => {
    ...
    const result = ctx.messageEdit.edit({ sessionId: agent.id, messageId: first.id, text: '第一条已改' })
    expect(result.ok).toBe(true)
  ...
  it('edits the same message repeatedly, targeting the latest replacement', async () => {
    ...
    const first = ctx.messageEdit.edit({ sessionId: agent.id, messageId: original.id, text: '第二版' })
    const second = ctx.messageEdit.edit({ sessionId: agent.id, messageId: original.id, text: '第三版' })
  ...
  it('fails closed without a live agent', async () => {
    ...
    const result = ctx.messageEdit.edit({ sessionId, messageId: 'm-1' as MessageId, text: 'x' })
  ...
  it('refuses a blank edit', async () => {
    ...
    const result = ctx.messageEdit.edit({ sessionId: agent.id, messageId: 'm-1' as MessageId, text: '   ' })
  ...
  it('refuses an over-long edit', async () => {
    ...
    const result = ctx.messageEdit.edit({ sessionId: agent.id, messageId: 'm-1' as MessageId, text: 'abcdef' })
  ...
  it('refuses an unknown message id and a non-user source', async () => {
    ...
    const unknown = ctx.messageEdit.edit({ sessionId: agent.id, messageId: ghost.id, text: 'x' })
    ...
    await agent.whenIdle()
    const nonUser = ctx.messageEdit.edit({ sessionId: agent.id, messageId: injected.id, text: 'x' })
