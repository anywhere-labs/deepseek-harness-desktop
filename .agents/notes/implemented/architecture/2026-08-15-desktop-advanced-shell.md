...
The native adapter separately reads the registered `ui-theme.preference` after Host boot settles and applies its built-in `light`, `dark`, or `system` value to Electron before constructing an advanced window. It observes committed preference changes for the active generation so the native frame and Windows material use the same appearance source as the built-in Client theme. Releasing the generation restores the preceding Electron appearance. A client-only third-party theme id has no Host preference to mirror and therefore does not change native appearance.

### Native materials

On macOS the advanced `BrowserWindow` uses `titleBarStyle: hiddenInset` and positioned traffic lights while leaving Electron transparency, vibrancy, and visual-effect options unset. The opaque renderer preserves the official sidebar-fill token and paints that fill on the desktop-owned sidebar surface. Wallpaper colors therefore do not alter sidebar contrast, and the renderer is not continuously composited over a native blur layer. Its 90 CSS-pixel collapsed column centers the official 56-pixel rail. The sidebar surface is non-draggable, while a desktop-owned unpainted strip above its content and to the right of the traffic lights provides a 32 CSS-pixel window drag target. A separate caption row reserves 20 CSS pixels above the conversation and details columns, while its unpainted native drag hit region also remains 32 CSS pixels high. The desktop shell therefore keeps the compact visual inset without inspecting or rearranging feature-owned Header nodes. Semantic controls and explicit no-drag contributions remain interactive; custom pointer targets in the top 32 pixels must opt out of the native drag region.

The desktop sidebar surface preserves the official opaque sidebar-fill token on macOS. The official sidebar and session list therefore keep the same component behavior, scrolling, spacing, fade, and fill as the Web client. Only the Windows surface scopes that token to transparent so the platform's Mica material remains visible.
...
Window-option and Electron-runtime tests verify macOS hidden-inset opaque chrome without transparency or vibrancy, Windows Mica/native controls, built-in native-theme initialization and live updates, generation-scoped appearance restoration, Linux rejection, and the tray's opposite-mode update. Client style tests verify the official sidebar fill on macOS and the scoped transparent fill on Windows. Shutdown tests verify relaunch only after successful zero-code disposal and no relaunch for a failed generation. Client and Host bundles build headlessly; graphical native appearance and target-machine performance remain a target-machine verification boundary.

### Target-machine performance investigation

A macOS Electron 43.4.0 / Chrome 150 target-machine A/B used two isolated synthetic 360-turn sessions. After all older history was loaded, each active conversation projected 1,080 chat nodes. The exact `origin/master` baseline and this opaque implementation were run with isolated `DSH_HOME` and user-data directories, automated session navigation, renderer/GPU process CPU accounting, and browser-level Chromium tracing. A no-trace repeat suppressed physical wheel input after one traced interval proved that external input can invalidate an idle sample.

The complete transparent chain added a reproducible cold-open cost: the first substantial session open took 427–439 ms in the baseline and 161–165 ms after this change, about 270 ms (62%) less in this fixture. Two hybrid runs—native transparent/vibrant window with opaque Client fill, and opaque native window with transparent Client CSS—both opened in 159–167 ms. The extra cold cost therefore required both the transparent `BrowserWindow` and transparent renderer surface; removing the complete pass-through composition path is a measured benefit of this decision.

The same experiment did not reproduce the reported 15–55% sustained idle renderer CPU. With tracing disabled and wheel input blocked, the baseline and opaque advanced renderer used 0.064% and 0.055% of one core while idle. Warm switching remained about 0.49 s in both implementations and saturated one renderer core. Compatibility mode reproduced the same behavior (471 ms p50 and 98% of one renderer core), and the trace attributes the dominant work to the unchanged upstream conversation view materializing every key in `order` via `ChatNodeSeat`. This PR therefore claims the visual fix and cold composition reduction only; conversation virtualization or retained mounted views belong to a separate upstream performance change. These target-machine figures characterize the synthetic fixture and do not define a release performance budget.
...
**Keep macOS sidebar vibrancy.** Native blur lets wallpaper colors enter the navigation surface, reducing the predictable contrast provided by the official Web fill, and adds a continuous compositor layer around large renderer updates. Opaque macOS chrome retains the advanced layout and hidden-inset controls while restoring deterministic theme colors and the simpler composition path.
...
DSH Desktop gains native-framed macOS and native-material Windows presentations without modifying the upstream submodule, copying the Web application, or introducing a second plugin or transport system. Tray changes and manual `settings.yaml` edits converge on one durable value, and a restart creates a coherent Host, Client, and native-window generation.
