...
Host 还会把当前 `zh` 或 `en` 界面语言投影为动态 system-prompt section。因此，除非用户明确要求其他语言，主代理及所有委派子代理都会使用所选界面语言输出解释、进度说明和最终答复；代码、标识符、命令、路径和直接引用保留原文。切换语言会立即影响后续模型步骤，无需重启应用或重新创建会话。
