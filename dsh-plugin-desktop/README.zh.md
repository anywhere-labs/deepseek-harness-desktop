...
Launcher 会在所有平台保留上游自适应目录选择器。在 Windows 上，安装阶段会修补 native picker package，使其通过 Koffi 直接解码所选 UTF-16 路径；这会避开 Electron 不支持的 external-buffer 操作，同时保留操作系统文件夹对话框。macOS 与 Linux 使用未经修改的上游选择结果。
