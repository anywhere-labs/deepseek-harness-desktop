...
export interface DesktopClientEnvironment {
  ...
  /** Whether the packaged Host exposes the stable update channel. */
  updates: boolean
}
...
export function parseDesktopClientEnvironment(search: string): DesktopClientEnvironment {
  ...
  const updates = params.get('dsh-desktop-updates')
  ...
  if (updates !== 'enabled' && updates !== 'disabled') {
    throw new Error(`dsh-plugin-desktop: invalid or missing dsh-desktop-updates ${JSON.stringify(updates)}`)
  }
  return {
