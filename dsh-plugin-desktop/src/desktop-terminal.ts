/** Isolated command-line environment launched from the AI Buddy tray. */
...
/** Platforms with a native terminal launch contract owned by AI Buddy. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`${PRODUCT_NAME} ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh(`Restart ${PRODUCT_NAME} after plugin changes.`)}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("${PRODUCT_NAME} {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart ${PRODUCT_NAME} after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(${PRODUCT_NAME} !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText(`Restart ${PRODUCT_NAME} after plugin changes.`)}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "${PRODUCT_NAME}" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
