...
Packaged macOS and Windows applications use `electron-updater` against stable GitHub Releases from `anywhere-labs/deepseek-harness-desktop`. The Host schedules the first check 60 seconds after activation and the next check six hours after each completed attempt. Manual checks, scheduled checks, downloads, cancellation, and installation share one controller. The updater disables automatic downloads, install-on-ordinary-quit, prereleases, downgrades, web installers, and differential downloads.

The packaged renderer receives an update marker in its same-origin loopback URL. It registers a **Software update** row in General settings and a dismissible notice when a release becomes available or finishes downloading. A fixed `/desktop-updates` loopback RPC accepts only state, check, download, cancel, install, and open-release-page operations with empty payloads. It returns version, progress, capability, and browser-safe error categories without exposing local paths, provider URLs, generic IPC, or Electron APIs. Development and ordinary Web clients do not register the update UI.

The settings row distinguishes idle, checking, current, available, downloading, downloaded, unsupported, and failure states. Failures identify an unreachable network, missing GitHub updater metadata, insufficient storage, an oversized download, cancellation cleanup, an installation that could not start, a browser launch failure, or an unknown provider error. The current state says explicitly when the installed build is the latest stable release. Missing `latest-mac.yml` or `latest.yml` offers the fixed GitHub Releases page instead of presenting a generic check failure.

Automatic installation requires capability metadata from both the running artifact and the target Release. macOS uses a signed and notarized application, DMG, ZIP, and `latest-mac.yml`. Windows uses NSIS, a blockmap, and `latest.yml`; signed artifacts can install in the app, while unsigned artifacts expose the GitHub download fallback. Downloads have a 1 GiB limit, a free-space reserve, and full-transfer byte enforcement. **Restart and update** hides the window, disposes the Host and its subprocesses, then calls the native updater handoff. Ordinary quit never installs a downloaded update.

Version 2.0.1 is the updater bootstrap and requires manual installation. Version 2.0.2 and later can use the complete in-app path when both packages support automatic installation. Release metadata is the discovery authority; the updater never pulls source, carries a GitHub token, or accepts an arbitrary URL.
...
- Linux uses the fixed GitHub download fallback until the release adds an AppImage updater target. Local Windows packages also use this fallback because they are unsigned; automatic Windows installation requires a release artifact with a valid Authenticode signature.
