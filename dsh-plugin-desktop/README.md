...
`yarn dist:linux` builds unsigned amd64 deb, rpm, and AppImage artifacts on a native Linux host. Like the macOS smoke, it refuses non-Linux and non-x64 hosts, runs the complete product gate (repository layout and community-contract checks, the Market build and check, then the Desktop build, every TypeScript compiler face, the full unit-test suite, runtime-closure verification, and CLI/Loader/profile headless smokes), then packages without signing material, sets `CSC_IDENTITY_AUTO_DISCOVERY=false`, and never publishes. It writes `DSH-Desktop-<version>-amd64.deb`, `DSH-Desktop-<version>-x86_64.rpm`, and `DSH-Desktop-<version>-x86_64.AppImage` under `dsh-plugin-desktop/dist/linux/`, and `verify-linux-package.ts` confirms each archive header plus the unpacked ELF executable at `dsh-plugin-desktop/dist/linux/linux-unpacked/dsh-desktop`. The rpm target shells out to the system `rpmbuild` executable (the `rpm` package), which the CI job installs before packaging; install it locally when building rpm. The artifacts target amd64 first, matching the Windows x64 installer; arm64 and snap remain out of scope.

Install the deb with `sudo dpkg -i DSH-Desktop-<version>-amd64.deb` (Debian/Ubuntu) or the rpm with `sudo rpm -i DSH-Desktop-<version>-x86_64.rpm` (Fedora/RHEL); the desktop entry launches the `dsh-desktop` binary from the app menu. The AppImage runs directly after `chmod +x DSH-Desktop-<version>-x86_64.AppImage` without installation. Linux runs compatibility mode only, so the window keeps the operating system frame and the official Web surface. The login-shell `PATH` recovery and the layered launch-environment snapshot already cover packaged Linux launches. The unpacked directory produced by `yarn package:dir` also runs directly at `dist/linux/linux-unpacked/dsh-desktop` without installing. The artifacts are unsigned, so some distributions may warn about an unknown publisher.
