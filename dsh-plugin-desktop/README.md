...
The desktop package has normal Host and Web Client faces. Its Client face validates the Host-supplied mode and platform markers in both modes. Both modes install the capability effects for renderer boot reporting, local folder drop, and submitted-input history; compatibility adds no services, slots, styles, or presentation, while advanced mode installs the desktop layout service and root presentation described below. Third-party Web clients continue to use the ordinary DSH module graph in both modes.
...
The desktop Client module validates the mode and platform markers, then performs no presentation replacement in compatibility mode. It does not provide or replace the `layout` service, register a `root` or `sidebar` occupant, or change the conversation surface. Desktop-owned boot-health reporting, local folder drop, and submitted-input history are capability effects; compatibility mode still preserves the selected profile's own layout, sidebar, and conversation composition, so the ordinary `desktop` and `web` profiles keep the official rows unchanged.

Each session rebuilds its history from loaded submitted prompts and slash-command lines, then appends inputs sent during the current desktop run. With the caret at the start of the composer, `Up` recalls older entries; while navigating, `Down` advances and restores the draft that was present before navigation after the newest entry. Open `@` and `/` candidate menus keep ownership of the arrow keys. The desktop client does not persist a separate history format.
