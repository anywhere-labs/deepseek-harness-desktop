...
The launcher keeps the upstream adaptive directory picker on macOS and Linux. On Windows it composes a Desktop-owned adapter over the official native picker and native client surface. The adapter starts the official dialog worker with Electron's supported Node subprocess mode, so local Desktop sessions open the system folder chooser without loading the N-API worker in the Electron main process. It preserves the ordinary `directoryPicker` capability seam, so workspace UI and third-party plugins use the same native behavior.
