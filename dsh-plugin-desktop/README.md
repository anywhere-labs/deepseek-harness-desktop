...
`yarn dist:linux` builds one unsigned amd64 deb on a native Linux host. Like the macOS smoke, it refuses non-Linux and non-x64 hosts, runs the complete product gate (repository layout and community-contract checks, the Market build and check, then the Desktop build, every TypeScript compiler face, the full unit-test suite, runtime-closure verification, and CLI/Loader/profile headless smokes), then packages without signing material, sets `CSC_IDENTITY_AUTO_DISCOVERY=false`, and never publishes. It writes `DSH-Desktop-<version>-amd64.deb` under `dsh-plugin-desktop/dist/linux/`, and `verify-linux-package.ts` confirms the deb archive header and the unpacked ELF executable at `dsh-plugin-desktop/dist/linux/linux-unpacked/dsh-desktop`. The package targets amd64 first, matching the Windows x64 installer; arm64, rpm, AppImage, and snap remain out of scope.

Install it with `sudo dpkg -i DSH-Desktop-<version>-amd64.deb`; the desktop entry launches the `dsh-desktop` binary from the app menu. Linux runs compatibility mode only, so the window keeps the operating system frame and the official Web surface. The login-shell `PATH` recovery and the layered launch-environment snapshot already cover packaged Linux launches. The unpacked directory produced by `yarn package:dir` also runs directly at `dist/linux/linux-unpacked/dsh-desktop` without installing. The deb is unsigned, so some distributions may warn about an unknown publisher.
