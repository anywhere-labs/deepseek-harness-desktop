...
The launcher retains the upstream adaptive directory picker on every platform. On Windows, the native picker package is patched at install time to decode the selected UTF-16 path directly through Koffi; this avoids the external-buffer operation unsupported by Electron while preserving the operating-system folder dialog. macOS and Linux use the unchanged upstream selection.
