...
The Host also projects the active `zh` or `en` interface locale into a dynamic system-prompt section. Main agents and every delegated subagent therefore use the selected interface language for explanations, progress updates, and final answers unless the user explicitly requests another language. Code, identifiers, commands, paths, and direct quotations retain their original text. A locale switch affects subsequent model steps immediately without restarting the application or recreating a session.
