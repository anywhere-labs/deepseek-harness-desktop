...
Both presentation modes reuse the existing loopback Web carrier. The profile mounts the ordinary `dsh-base` and `dsh-web-app` bundles, the Host binds its HTTP and WebSocket surface to `127.0.0.1` on an ephemeral port, and Electron loads that same-origin page in a sandboxed renderer. One narrow preload bridge maps a renderer `File` supplied by the user to Electron's native source path so attachment plugins can retain that source; it accepts no string path and exposes no Node, filesystem, or IPC primitive. There is no Electron-owned plugin roster or raw Electron API in the renderer.
